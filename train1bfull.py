...
BETA1 = 0.5
OUTPUT_DIR = './gan_outputs/1b_full'
...
sample = softmin_sample_func(filter_radii, 0.01, device)


class BlurredImageSampler(torch.utils.data.Dataset):
    def __init__(self, blurred_tensor_path, labels_path="./data"):
        self.all_blurred = torch.load(blurred_tensor_path)[0: 2]  # shape [N, B, 1, 28, 28]
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (imgs, _, blur_levels) in enumerate(dataloader):
            ...
            for _ in range(1):
                ...
                blur_radii = sample(imgs.size(0)).to(device) / 27.0 # Uniform distribution
