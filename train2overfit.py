...
BETA1 = 0.5
OUTPUT_DIR = './gan_outputs/2_overfit'
...
class BlurredImageSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        blur_idx = sample(1).item()
        img = self.all_blurred[idx%self.N, blur_idx]
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (imgs, _, blur_levels) in enumerate(dataloader):
            ...
            for _ in range(1):
                ...
                blur_radius = sample(1).to(device)
                t = (blur_radius/27.0).repeat(imgs.size(0), 1)
                
                # Generate images
                generated_imgs = apply_gaussian_blur(generator(z), blur_radius)
                
                # Generator loss
                g_loss = adversarial_loss(discriminator(generated_imgs, t), valid)
            ...
            # Generate images
            generated_imgs = apply_gaussian_blur(generator(z), blur_radius)
            fake_loss = adversarial_loss(discriminator(generated_imgs, t), fake)
