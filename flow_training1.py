...
OUTPUT_DIR = f'./gan_outputs/flow/1a_baseline_epochs_{EPOCHS}'
...
def train_gan():
    ...
    # t_func, e_func = cosine_flow_funcs(EPOCHS, cool_down=EPOCHS*0.1)
    t_func, e_func = constant_flow_funcs(0)
    # print([t_func(i) for i in range(EPOCHS)])
    ...
    for epoch in range(EPOCHS):
        ...
        # write(str((t, e)))
        for imgs in dataloader:  # imgs: [batch, B, 1, 28, 28]
            imgs = imgs.to(device)
            ...
            similar_generated_imgs = generator(z, blur_radii + EPSILON) # this epsilon is related to continuity
            ...
            real_loss = adversarial_loss(discriminator(selected_imgs, blur_radii), valid)# (min([( epoch / EPOCHS / WARM_UP), 1])) * adversarial_loss(discriminator(imgs, blur_levels), valid)
        ...
        if epoch % 5 == 0:
            ...
            generate_and_plot_at(generator, epoch, t=t)
    ...
    generate_and_plot_at(generator, EPOCHS , t=0)
    # Plot and save loss curves
    plt.figure(figsize=(10, 5))
    plt.plot(g_losses, label='Generator Loss')
    plt.plot(d_losses, label='Discriminator Loss')
...
def generate_and_plot_at(generator, epoch, num_images=100, t=0):
    # Set generator to evaluation mode
    generator.eval()
    ...
    with torch.no_grad():
        z = torch.randn(num_images, LATENT_DIM).to(device)
        blurs = t * torch.ones(num_images).to(device)
        generated_imgs = generator(z,blurs).to(device).cpu()
    
    # Prepare output path
    output_path = os.path.join(OUTPUT_DIR, f'generated_image_epoch_{epoch}.png')
